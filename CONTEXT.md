...
- [x] 底盘改四舵轮
- [x] 删除风车发球机（接球机不需要）
- [~] 俯仰从齿轮改杠杆（标定表已采集、坐标变换层已实现，待编译验证 + 现场调 hold/seat 前馈）
...
- Delta 臂完整运动学还缺两个几何量：静平台半径 R（电机轴到平台中心）和动平台半径 r（球铰到末端中心），待量取

## 硬件实测参数（已量取）

- Delta 臂主动臂（下臂，电机驱动杆）长度 **210.5 mm**；从动臂（平行四边形连杆）长度 **215 mm**；主动臂最大上抛行程 **77° = 1.344 rad**（2026-07-13 实测）
- 俯仰杠杆标定表（2026-07-13 实测，物理倾角° → 电机相对零点角°，共 11 点）：7→27.230，12→16.030，15→7.588，19→-2.310，22→-10.386，26→-21.068，30→-33.212，34→-45.500，36→-56.381，38→-65.540，40→-80.408。单调递减；**抬头=电机负方向（与旧齿轮方向相反）**；电机 0°≈物理倾角 18°；40° 附近斜率陡增（~-7.4）。完整表也备份在 `arm_config.yaml` tilt 段注释。坐标变换代码待讨论后实现。

---

## 最近变更

- 2026-07-13: [俯仰待机位改中间 + 软着陆缓降] ① 俯仰待机位从压底位（0）改为中间位：新配置键 `tilt.ready_tilt_rad: 0.19`（相对压底零点的物理倾角增量，与 topic 同语义，构造时查表转电机角存 `tilt_ready_motor_rad_`；旧键 `ready_angle_rad` 兼容）。**零点坐标不变（压底最低点=0）**：压底锁零后主动抬到待机位，READY/RECOVER_READY 都回该位；置 0 恢复旧行为。② 三路 delta 软着陆加向上缓降前馈：`initialization.downward_torque: -0.00 → 0.35`（正=向上托，无代码改动）；⚠️ 上限 ~0.6，太大臂悬停会被速度判据误判着陆、零点锁半空。⚠️ build 通道持续故障，所有批次改动均**未编译**。

- 2026-07-13: [退出泄力 + 待机震动排查] ① `delta_arm_manager_node` 新增 `sendReleaseAll()`：节点退出（Ctrl+C 后 main 显式调用 + 析构兜底，`release_sent_` 防重）向 3 路 delta + 俯仰共 4 路 GO8010 发 kp=kd=τ=0 泄力帧，重发 3 轮×20ms 间隔，防止退出后电机保持最后一帧 PD 顶着机械。**关机顺序：先关 delta 节点、后关 motor_control_node**（驱动节点死了泄力帧发不到串口）。② 现象"启动 delta 节点即震动/嗡嗡"：头号嫌疑为待机位 `tilt.hold_torque_ff` 把臂顶离支撑与 PD 打架，已从 -1.5 降到 **-0.5** 起调（仍震→再减/置 0；抬起后往下掉→加大）。`ArmTarget.msg` 注释同步新语义（相对物理倾角，域约 [-0.19,+0.38] rad）。⚠️ build 通道故障，本批与上批改动均**未编译**。

- 2026-07-13: [俯仰杠杆坐标变换实现] 按实测标定表（11 点，见「硬件实测参数」）实现杠杆非线性变换：① `arm_config.yaml` tilt 段新增 `calibration` 表（[绝对倾角°, 电机角°]）+ 压底找零参数（`seat_torque_ff: 0.3`/`seat_duration_s: 0.5`/`seat_kd: 0.05`）；`hold_torque_ff` 翻负为 **-1.5**（杠杆机构抬头=电机负方向，与旧齿轮相反），幅值沿用旧值**现场必调**。② `delta_arm_manager_node`：新增 `tiltRelToMotorRel()`（分段线性插值，超域 [7°,40°] 钳位 WARN，未配表退回电机角直传）；`ArmTarget.tilt_angle_rad` 语义改为**相对压底位姿的物理倾角增量**（rad，正=抬头），callback 内查表转电机角，之后滑动/到位判定仍在电机空间（tiltCommand 未动）；俯仰零点从"首帧反馈锁零"改为 **controlLoop 压底找零**（向下前馈压实躺平支撑 0.5s 再锁零，保证杠杆基准可重复，40° 端零点旷量被 ~7 倍放大）。压底位绝对倾角≈18.07°（表内电机 0° 交点，加载时自动求出）。⚠️ **尚未编译**（build 通道临时故障），待 `colcon build` 验证；上层发 tilt_angle_rad 的节点注意语义已变。

- 2026-07-13: [Delta 臂几何实测 + 行程上限更新] 实测记录：主动臂（下臂）210.5 mm、从动臂 215 mm（完整运动学还缺静平台/动平台半径）。最大上抛行程实测 77°，`arm_config.yaml` 三路 `max_delta_rad` 从 1.612/1.611/1.567 统一改为 **1.344 rad**（旧 ~92° 数据作废），`delta_arm_manager_node.cpp:64` 兜底默认值同步改 1.344。注意：行程缩短后上抛峰速理论值降为 √(2×150×1.344)≈20.1 rad/s（原 ~21.9），出手速度略降，不够再抬 max_acceleration。已 colcon build 装入 install。
