...
## 每轮执行步骤
1. **INPUT**: 从 `copilot_enhance_3212` 协议同步工具的上一次返回中读取 `user_input`
2. **THINK & ACT**: 使用copilot 原生工具执行任务
3. **REPORT**: 在copilot中以**中文**输出执行结果和思考过程
4. **SYNC**: 调用 `copilot_enhance_3212` 协议同步工具，将流程交还用户
...
| **用户反馈** | **`copilot_enhance_3212`** | **每轮必须调用，禁止遗漏** |
...
## 流程保障
1. 每轮回复的最终动作需为 `copilot_enhance_3212` 协议同步工具调用，不以文字输出收尾
